...
async function test() {
  ...
  await canvas0.screenshot({ path: '/tmp/test-frame3-0.jpg', type: 'jpeg', quality: 75 });
  console.log('Saved /tmp/test-frame3-0.jpg');
  ...
  await canvas50.screenshot({ path: '/tmp/test-frame3-50.jpg', type: 'jpeg', quality: 75 });
  console.log('Saved /tmp/test-frame3-50.jpg');
  
  // Compare MD5s
  const file0 = await fs.readFile('/tmp/test-frame3-0.jpg');
  const file50 = await fs.readFile('/tmp/test-frame3-50.jpg');
  ...
  console.log('File sizes:', file0.length, file50.length);
